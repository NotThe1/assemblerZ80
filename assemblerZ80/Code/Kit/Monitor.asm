; Monitor.asm
; Monitor source code for Z80 Trainer Kit
; The original source code was for MPF-1
;

; 5 December 2014 add brightness adjustment for the display
; 6 December 2014 test serial TxD at 2400 bit/s
; 7 December 2014 test hex file download
; 12 December 2014 test upload and download hex file
;                  adjust monitor locations compatible with MPF-1
; 16 January 2015 change start text to CPU80
;                 remove display Accumulator in NMI
; 13 February 2015 fix serial port for 2400 bit/s
;                  now xtal frequency is 3.58MHz
;
;***********************************************************
;
;
;.FILL	0800H, 0FFH	;(Required to trick assembler into filling empty space with FFh bytes)

;
P8255	EQU		03H				;8255 I control port
DIGIT	EQU		01H				;8255 I port C
SEG7	EQU		02H				;8255 I port B
KIN		EQU		00H				;8255 I port A
PWCODE	EQU		0A5H			;Power-up code
ZSUM	EQU		71H				;This will make the sum of all
								;monitor codes to be zero

GPIO1   EQU    40H				; 8-bit LED display
...
COLDEL	EQU		201		;Column delay time for routine SCAN and SCAN1
F1KHZ	EQU		65		;Delay count for 1 kHz square wave, used by routine TONE1K.
F2KHZ	EQU		31		;Delay count for 2 kHz square wave, used by routine TONE2k.
MPERIOD	EQU		42		;1 kHz and 2 kHz threshold, used by tape input routine PERIOD.
...
;***********************************************************
RST00:	ORG		00H		;Required to make sure assembler doesn't skip over address 0000H; original comment: -- reset --
...
;
RESET1:
		LD		(USERIF),HL ;set user's I register and interrupt flip flop to 0
...
; of user's SP.
		...
		;OUT     (GPIO1),A		; display ACC at GPIO1
		NOP
		NOP
		LD		A, 10010000B
...
RGSAVE:
		LD		(HLTEMP), HL	;save register HL
		POP		HL		;get return address from stack
		LD		(ADSAVE), HL	;Save return address into ADSAVE.
		LD		(USERPC), HL	;Set user's PC to return address.
...
; is done by the following instructions.
		...
		JR		C, SETST0
		LD		IX, DISPBF
		SCF			;set carryflag to indicate the user's SP is legal.
		JR		BRRST0
;
SETST0:
...
; two instructions set STATE to zero.
		...
		LD		(STATE), A
BRRST0:
		LD		A, (BRDA)	;restore the data at break address
		LD		HL, (BRAD)
		LD		(HL), A
...
MAIN:
		...
		CALL	SCAN		;Scan display and input keys.
							;Routine SCAN will not return until any key is pressed.
		CALL	BEEP		;After a key is detected, there
							;will be accompanied with a beep sound.
...
;Executed when 'DATA' key is pressed.
	...
	RET
TESTRG:
	CP	8		;check if the status is 8 or 9
				;(RGAD or RGDA).
...
IMV:	LD	HL, STMINOR	;STMINOR contains the
	...
	JP	IGNORE
ISTEP:
	CALL	STEPDP		;Display the parameter at
				;step buffer.
	RET
;
IRGAD:
IRGDA:	LD	HL, STMINOR	;In these states, the STMINOR
	...
	LD	(HL), 0
IRGNA:
	CALL	REGDP9		;Display the register and
				;set STATE to 9
	RET
;
;***********************************************************
...
;***********************************************************
GRL:
	LD	DE, (STEPBF)	;Load starting address
				;into DE.
...
ENDTAPE:
	LD	DE, (STEPBF+4)	;DE = last address
	JR	ENDFUN		;Continue at ENDFUN
;
ERROR:
	LD	IX, ERR_	;IX points to '-Err  '
	JP	SETST0		;Set STATE to 0 by
				;branching to SETST0.
;
;***********************************************************
...
LEAD1:
:
	CALL	PERIOD		;The return of PERIOD
				;is in flag:
...
FLAGX:
	...
	JR	NC, FCONV2
FCONV1:
	LD	A, (USERAF)	;Get user's F register
	...
	RET
FCONV2:
	LD	HL, (FLAGH)	;Get the binary form
				;of 4 upper bits of
				;user's F register.
...
; Registers AF, B, HL are destroyed.
	...
	LD	B, 4		;Loop 4 times.
ERL4:
	ADD	HL, HL		;Shift HL left 4 bits.
...
; Destroyed registers: AF, BC, DE, HL, AF', BC', DE', HL'
	...
	EX	AF, AF'
TLOOP:
	CALL	GETBYTE		;Read 1 byte from tape.
...
; Registers destroyed: AF, DE
	...
	LD	DE, 0
LOOPH:
	IN	A, (KIN)	;Bit 7 of port A is Tapein.
	INC	DE
	RLA
...
;last one will be returned.
	...
	LD	E, 00000001B	;Scan from rightmost digit
	LD	H, 6
...
;	DJNZ	$		;Delay 1.5 ms per digit.
	...
	LD B,50H	; compensate LED brightness control
	DJNZ $
...
KROW:
	RR	D		;Rotate D 1 bit right, bit 0
...
NOKEY:
	INC	C		;Increase current key-code by 1.
...
SUMOK:
	RST	00H		;Display 'uPF--1'.
INI3:
	LD	(POWERUP), A	;Load power-code into
				;(POWERUP).  The monitor
...
BIT_DELAY: LD B,31H      ; old value 12H for 330pF
...
; DISPLAY 16 BYTES START AT CURRENT PC
; ENTRY: HL POINTED TO MEMORY ADDRESS TO BE DISPLAYED
...
NEXT_BIT_RX:
  CALL BIT_DELAY
	  ...
	  CALL BIT_DELAY    ; CENTER OF STOP BIT
...
; DELAY 1.5 BIT FOR 2400 BIT/S
BIT1_5_DELAY:
 LD B,4DH      ; old value 20H with 330pF
           NOP              ; 4DH is for 100pF 805Hz (800Hz)
           NOP
           NOP
	   NOP
           DJNZ $
...
; UPLOAD HEX FILE
; EXECUTE AFTER KEY TAPE WRITE + GO 
;
...
UPLOAD3:
	...
	LD B,50		; DELAY BETWEEN CHARACTER
	DJNZ $
...
SKIP10:
	...
	CALL OUT2X   ; SEND BYTE CHECK SUM
...
GET_RECORD1:
 CALL CIN
	   ...
	   XOR A	; CLEAR BYTE CHECK SUM	
	   LD (BCC),A
	 
	   CALL GET_HEX
	   LD C,A	; GET NUMBER OF BYTE
...
DATA_RECORD:
	   ...
	   INC HL
	   DEC C
	   JR NZ,DATA_RECORD ; UNTIL C=0

	   LD A,(BCC)
	   NEG
	   LD (BCC),A
	   CALL GET_HEX	   ; GET BYTE CHECK SUM
	   
	   LD E,A

	   LD A,(BCC)

	   CP E		; COMPARE WITH BYTE CHECK SUM
